...
data:
  ...
  code: "#pragma once\n#include <bits/stdc++.h>\n\n#include \"src/FFT/NTT.hpp\"\n\n\
    ...
    \ + k, 0, s = std::min(l, n - k)),\n            gt1[i].zeros(s, l2), gt1[i].dft(0,\
    \ l2);\n      if (&p != &q)\n        for (int i = 0, k = 0, s; k < m; i++, k +=\
    \ l)\n          gt2[i].set(q.data() + k, 0, s = std::min(l, m - k)),\n       \
    \       gt2[i].zeros(s, l2), gt2[i].dft(0, l2);\n      else\n        for (int\
    \ i = 0; i < nn; i++) gt2[i].subst(gt1[i], 0, l2);\n      gt2[mm].zeros(0, l2);\n\
    \      for (int i = mm; i--;)\n        gt2[i + 1].add(gt2[i], 0, l), gt2[i + 1].dif(gt2[i],\
    ...
    \ 0, l2),\n              GNA2::bf.add(GNA1::bf, 0, l2);\n        GNA2::bf.idft(0,\
    \ l2), GNA2::bf.get(rr + k, 0, std::min(l, sz - k));\n      }\n    } else {\n\
    ...
    \          for (rr[i] = 0, j = i - m + 1; j < n; j++) rr[i] += pp[j] * qq[i -\
