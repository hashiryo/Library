...
data:
  ...
  attributes:
    ...
    links: []
  bundledCode: "#line 2 \"src/Old/BigInt.hpp\"\n#include <bits/stdc++.h>\n#line 3\
    \ \"src/Old/ModInt.hpp\"\n/**\n * @title ModInt\n * @category \u6570\u5B66\n */\n\
    ...
    \ std::string &s) {\n    neg = false, dat.clear();\n    int pos = 0;\n    for\
    \ (; pos < (int)s.size() && (s[pos] == '-' || s[pos] == '+'); ++pos)\n      if\
    \ (s[pos] == '-') neg = !neg;\n    for (long long i = s.size() - 1, x = 0; i >=\
    \ pos; i -= bdig, x = 0) {\n      for (int j = std::max<int>(pos, i - bdig + 1);\
    \ j <= i; j++)\n        x = x * 10 + s[j] - '0';\n      dat.push_back(x);\n  \
...
layout: document
redirect_from:
- /library/src/Old/BigInt.hpp
- /library/src/Old/BigInt.hpp.html
title: "\u591A\u500D\u9577\u6574\u6570"
---
