...
data:
  ...
  code: "#pragma once\n#include <vector>\n#include <algorithm>\n#include <array>\n\
    ...
    \ &os, const Node &t) { return os << \"{z:\" << t.z << \",x:\" << t.x << \",d:\"\
    ...
    \ if (t) push(t), dump_xs(t->ch[0], xs), xs.push_back(t->x), dump_xs(t->ch[1],\
    ...
    \ ol - ou;\n  else return ou - D(x) * ol;\n }\n np mn;\n bool bf[2];\n T o[2],\
    \ rem, bx[2];\n D y;\n D calc_y(T x) {\n  if (!mn) return 0;\n  if (mn->x == x)\
    \ return 0;\n  return push(mn), x < mn->x ? calc_y<0>(mn->ch[0], x, o[0], D(mn->x)\
    \ * o[0]) : calc_y<1>(mn->ch[1], x, o[1], D(mn->x) * o[1]);\n }\n void slope_lr(bool\
    \ r) {\n  np t= mn;\n  if (!t) return;\n  for (; push(t), t->ch[r];) t= t->ch[r];\n\
    \  D p= sum(mn->ch[r]) + D(mn->x) * o[r];\n  T q= o[r] + sl(mn->ch[r]);\n  splay(mn=\
    \ t), o[r]= 0, o[!r]= t->d, r ? (y-= p, rem+= q) : (y+= p, rem-= q);\n }\n void\
    \ slope_eval() {\n  if (rem == 0 || !mn) return;\n  bool neg= rem < 0;\n  T p=\
