...
data:
  ...
  bundledCode: "#line 2 \"src/Math/SetPowerSeries.hpp\"\n#include <algorithm>\n#include\
    ...
    \ 21> struct SetPowerSeries {\n#define SUBSET_REP(i, j, n) \\\n for (int _= (n),\
    \ _e= (n); _>>= 1;) \\\n  for (int __= 0, _2= _ << 1; __ < _e; __+= _2) \\\n \
    \  for (int j= __, i= j | _, ___= i; j < ___; ++j, ++i)\n template <typename T>\
    \ static inline void ranked_zeta_tr(const T f[], T ret[][MAX_N + 1], const int\
    \ sz) {\n  for (int S= sz, c; S--;) ret[S][c= __builtin_popcount(S)]= f[S], std::fill_n(ret[S],\
    ...
    \ // O(n 2^n)\n template <typename T> static inline void subset_sum(std::vector<T>\
    \ &f) { SUBSET_REP(S, U, f.size()) f[S]+= f[U]; }\n // O(n 2^n)\n template <typename\
    \ T> static inline void subset_sum_inv(std::vector<T> &f) { SUBSET_REP(S, U, f.size())\
    ...
    \ sz == (int)g.size());\n  return conv_tr(f.data(), g.data(), ret.data(), sz),\
    \ ret;\n }\n // f(S) = \u03C6_S ( \u03A3_{T\u228AS} f(T)g(S/T) )   O(n^2 2^n)\n\
    ...
    \ as binary numbers) } f(T)f(S/T) )   O(n^2 2^n)\n template <class T, class F>\
    ...
    \ T, class EGF> static inline std::vector<T> composite(const std::vector<T> &f,\
    ...
    \  O(n^2 2^n)\n template <class T> static inline std::vector<T> log(const std::vector<T>\
...
| `semi_relaxed_convolve(g, init, phi)` | $\displaystyle  f_S=\begin{cases}  \text{init} & S = \varnothing, \newline \phi_S\left(\sum_{T \subsetneq S} f_Tg_{S\backslash T}\right)& S \neq \varnothing \end{cases}$<br> となる $f$ を返す. <br> 実際は $\phi_S$ は `phi(int,T&)` で参照渡しの関数を与える. | $\mathcal{O}(n^22^n)$    |
| `self_relaxed_convolve<T>(n, phi)`    | $\displaystyle f_S=\begin{cases} 0 & S= \varnothing, \newline \phi_S\left(\frac{1}{2}\sum_{\varnothing \neq T \subsetneq S}  f_Tf_{S\backslash T}\right) & S \neq \varnothing \end{cases}$<br> となる $f$ を返す. <br> 実際は $\phi_S$ は `phi(int,T&)` で参照渡しの関数を与える.　<br> | $\mathcal{O}(n^22^n)$    |
| `composite(f,F)`  | $\displaystyle  F(f) = \sum_{i=0}^{\infty} \frac{F_i}{i!} f^i $ を返す. <br> $f_{\varnothing}=0$ でないと assert で落ちる. | $\mathcal{O}(n^22^n)$    |
| `exp(f)`| $\displaystyle  \exp(f) =  \sum_{i=0}^{\infty} \frac{1}{i!} f^i $ を返す. <br> $f_{\varnothing}=0$ でないと assert で落ちる.    | $\mathcal{O}(n^22^n)$    |
| `log(f)`| $ \log f $ を返す. <br> $f_{\varnothing}=1$ でないと assert で落ちる.    | $\mathcal{O}(n^22^n)$    |
| `pow(f,k)`   | $f^k$ を返す.   | $\mathcal{O}(n^22^n)$    |
| `polynomial_composite(f,P)` | 多項式 $\displaystyle  P(x) = \sum_{i=0}^{m-1}P_ix^i $ に対して $P(f)$ を返す.   | $\mathcal{O}(n^22^n+nm)$ |
| `egf(f)`| $k=0,1,\dots,n$ について<br> $\displaystyle \left\lbrack X^{2^n-1}\right\rbrack\frac{1}{k!}f^k$<br> を返す.  | $\mathcal{O}(n^22^n)$    |
| `egf(f,g)`   | $k=0,1,\dots,n$ について <br>$\displaystyle \left\lbrack X^{2^n-1}\right\rbrack\frac{1}{k!}gf^k$<br> を返す. | $\mathcal{O}(n^22^n)$    |
