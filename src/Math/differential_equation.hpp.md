...
data:
  ...
  bundledCode: "#line 2 \"src/Math/differential_equation.hpp\"\n#include <bits/stdc++.h>\n\
    ...
    \ (int m = lim; m < n / 2; m *= 2) {\n      mod_t dw = G.pow((mod - 1) / (4 *\
    \ m));\n      for (int i = 0; i < m; i++) W[m + i] = W[i] * dw;\n      lim = n\
    \ / 2;\n    }\n    for (int m = n; m >>= 1;)\n      for (int s = 0, k = 0; s <\
    \ n; s += 2 * m, ++k)\n        for (int i = s, j = s + m; i < s + m; ++i, ++j)\
    \ {\n          mod_t u = x[i], v = x[j] * W[k];\n          x[i] = u + v, x[j]\
    \ = u - v;\n        }\n  }\n  static inline void crt(m64_1 f1[], m64_2 f2[], int\
    ...
    };\n#line 5 \"src/Math/differential_equation.hpp\"\n/**\n * @title \u5FAE\u5206\
    ...
    \ (int e = 1, ne = 2; e < deg; e = ne, ne = min(e << 1, deg)) {\n    auto a =\
    \ dF(f, ne - 1), r = a.inte().exp(), h = a * f;\n    h.resize(ne - 1), h = (F(f,\
...
layout: document
redirect_from:
- /library/src/Math/differential_equation.hpp
- /library/src/Math/differential_equation.hpp.html
title: "\u5FAE\u5206\u65B9\u7A0B\u5F0F"
---
