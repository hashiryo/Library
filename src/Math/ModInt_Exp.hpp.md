...
data:
  ...
  bundledCode: "#line 2 \"src/NumberTheory/Factors.hpp\"\n#include <numeric>\n#include\
    ...
    \ Uint= std::conditional_t < MOD<(1ull << 32), uint32_t, uint64_t>;\n using DUint=\
    \ std::conditional_t<std::is_same_v<Uint, uint64_t>, __uint128_t, uint64_t>;\n\
    \ using mod_t= ModInt_Exp;\n static constexpr inline Uint mod(DUint x) { return\
    \ x < MOD * 2 ? Uint(x) : Uint(x % MOD) + MOD; }\n static constexpr inline Uint\
    \ mul(Uint a, Uint b) { return mod(DUint(a) * b); }\n static constexpr inline\
    \ Uint pow(Uint b, Uint k) {\n  for (Uint ret(1);; b= mul(b, b))\n   if (k & 1\
    \ ? ret= mul(ret, b) : 0; !(k>>= 1)) return ret;\n }\n static constexpr inline\
