...
data:
  ...
  bundledCode: "#line 2 \"src/LinearAlgebra/LU_Decomposition.hpp\"\n#include <type_traits>\n\
    ...
    #line 5 \"src/LinearAlgebra/LU_Decomposition.hpp\"\nnamespace la_internal {\n\
    ...
    \ i= s + 1; i < ed; ++i) a[i]^= b[i];\n}\nvoid add_lower(u128 *a, const u128 *b,\
    \ size_t ed) {\n size_t s= ed >> 7;\n a[s]^= b[s] & ((u128(1) << (ed & 127)) -\
    \ 1);\n for (size_t i= s; i--;) a[i]^= b[i];\n}\nvoid subst_lower(u128 *a, const\
    \ u128 *b, size_t ed) {\n size_t s= ed >> 7;\n a[s]= b[s] & ((u128(1) << (ed &\
    \ 127)) - 1);\n for (size_t i= s; i--;) a[i]= b[i];\n}\nbool any1_upper(const\
    \ u128 *a, size_t bg, size_t ed) {\n size_t s= bg >> 7;\n if (a[s] & -(u128(1)\
    ...
    \ h; ++r) {\n    auto a= A[perm[r]];\n    bool m= a[c];\n    if (a[c]= 0, a[psz]=\
    \ m; m) add_upper(a.data(), b.data(), c, w);\n   }\n   piv[psz++]= c;\n  }\n \
    ...
    \ h= dat.height(), w= dat.width(), n= rank();\n  assert(h == b.size());\n  Vector<bool>\
    \ y(h), x(w);\n  for (size_t c= 0; c < h; ++c)\n   if (y[c]^= b[perm[c]]; c <\
    \ w && y[c]) add_upper(y.data(), dat[c].data(), c, h);\n  if (any1_upper(y.data(),\
  ...
  code: "#pragma once\n#include <type_traits>\n#include <numeric>\n#include \"src/LinearAlgebra/Matrix.hpp\"\
    ...
    \ < 1e-8;\n  else return x == K();\n }\npublic:\n LU_Decomposition(const Matrix<K>\
    \ &A): dat(A), perm(A.height()), sgn(false), psz(0) {\n  const size_t h= A.height(),\
    ...
    \ std::swap(perm[pos], perm[psz]);\n   const auto b= A[perm[psz]];\n   for (size_t\
    \ r= psz + 1; r < h; ++r) {\n    auto a= A[perm[r]];\n    bool m= a[c];\n    if\
    \ (a[c]= 0, a[psz]= m; m) add_upper(a.data(), b.data(), c, w);\n   }\n   piv[psz++]=\
    \ c;\n  }\n  for (size_t j= w; j--;)\n   for (size_t i= h; i--;) dat[j][i]= A[perm[i]][j];\n\
    \ }\n size_t rank() const { return psz; }\n bool is_regular() const { return rank()\
    \ == dat.height() && rank() == dat.width(); }\n bool det() const { return is_regular();\
    \ }\n std::vector<Vector<bool>> kernel() const {\n  const size_t w= dat.width(),\
