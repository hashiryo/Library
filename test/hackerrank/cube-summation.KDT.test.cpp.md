...
data:
  ...
  bundledCode: "#line 1 \"test/hackerrank/cube-summation.KDT.test.cpp\"\n#define PROBLEM\
    ...
    #line 9 \"src/DataStructure/KDTree.hpp\"\ntemplate <uint8_t K, class pos_t, class\
    \ M> class KDTree {\n HAS_MEMBER(op);\n HAS_MEMBER(ti);\n HAS_MEMBER(mapping);\n\
    ...
    \ sg_, bool du_> struct Node_D<sg_, du_, typename std::enable_if_t<!sg_ && du_>>:\
    ...
    \  if (ed - bg < 1) return;\n  const int n= ed - bg;\n  auto md= bg + n / 2;\n\
    \  std::nth_element(bg, md, ed, [div](const PosVal &l, const PosVal &r) { return\
    \ l.first[div] < r.first[div]; });\n  ns[i= ts++].val= md->second;\n  for (uint8_t\
    \ j= K; j--; ns[i].pos[j]= md->first[j]) {\n   auto [mn, mx]= std::minmax_element(bg,\
    \ ed, [j](const PosVal &l, const PosVal &r) { return l.first[j] < r.first[j];\
    \ });\n   ns[i].range[j][0]= mn->first[j], ns[i].range[j][1]= mx->first[j];\n\
    \  }\n  if (uint8_t nex= (div + 1) % K; n > 1) build(ns[i].ch[0], bg, md, ts,\
    \ nex), build(ns[i].ch[1], md + 1, ed, ts, nex);\n  if constexpr (monoid<M>::value)\
    ...
    \ (uint8_t j= K; j--; myself&= pos[j] == ns[i].pos[j])\n   if (ns[i].range[j][1]\
  ...
  code: "#define PROBLEM \"https://www.hackerrank.com/challenges/cube-summation\"\n\
    ...
    \ x1, y1, z1, x2, y2, z2});\n   }\n  }\n  KDT kdt({mp.begin(), mp.end()});\n \
    \ for (auto q: query) {\n   if (q[0] == 0) {\n    auto [_, x, y, z, w, __, ___]=\
    \ q;\n    kdt.set(w, {x, y, z});\n   } else {\n    auto [_, x1, y1, z1, x2, y2,\
