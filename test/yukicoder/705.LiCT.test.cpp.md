...
data:
  ...
  bundledCode: "#line 1 \"test/yukicoder/705.LiCT.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/705\"\
    ...
    #line 5 \"test/yukicoder/705.LiCT.test.cpp\"\nusing namespace std;\nsigned main()\
    \ {\n cin.tie(0);\n ios::sync_with_stdio(0);\n int n;\n cin >> n;\n long long\
    \ a[n], x[n], y[n];\n for (int i= 0; i < n; ++i) cin >> a[i];\n for (int i= 0;\
    \ i < n; ++i) cin >> x[i];\n for (int i= 0; i < n; ++i) cin >> y[i];\n auto f=\
    \ [](long long x) { return abs(x * x * x); };\n auto w= [&](int i, int j, long\
    \ long d) { return d + f(x[j] - a[i - 1]) + f(y[j]); };\n LiChaoTree lct(w, 1,\
  ...
  code: "#define PROBLEM \"https://yukicoder.me/problems/no/705\"\n#include <iostream>\n\
    ...
    \ i, int j, long long d) { return d + f(x[j] - a[i - 1]) + f(y[j]); };\n LiChaoTree\
    \ lct(w, 1, n + 1);\n auto tree= lct.make_tree<MINIMIZE>();\n tree.insert(0, 0);\n\
    \ for (int i= 1; i < n; ++i) {\n  auto dp= tree.query(i).first;\n  tree.insert(i,\
    \ dp);\n }\n cout << tree.query(n).first << '\\n';\n return 0;\n}"
  dependsOn:
  ...
  verifiedWith: []
documentation_of: test/yukicoder/705.LiCT.test.cpp
layout: document
redirect_from:
- /verify/test/yukicoder/705.LiCT.test.cpp
- /verify/test/yukicoder/705.LiCT.test.cpp.html
