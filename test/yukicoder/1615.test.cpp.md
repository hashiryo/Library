...
data:
  ...
  bundledCode: "#line 1 \"test/yukicoder/1615.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/1615\"\
    ...
    \ Y, Z;\n  cin >> X >> Y >> Z, X--, Y--;\n  graph.add_edge(buyer[X], product[Y],\
    \ 0, 1, (1ll << Z));\n }\n for (int i= 0; i < N; i++) graph.add_edge(gs, buyer[i],\
    \ 0, 1, 0);\n for (int j= 0; j < M; j++) graph.add_edge(product[j], gt, 0, 1,\
    \ 0);\n graph.add_edge(gs, gt, 0, N + M, 0);\n graph.add_supply(gs, N + M), graph.add_demand(gt,\
