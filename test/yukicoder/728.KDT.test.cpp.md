...
data:
  ...
  bundledCode: "#line 1 \"test/yukicoder/728.KDT.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/728\"\
    ...
    #line 6 \"test/yukicoder/728.KDT.test.cpp\"\nusing namespace std;\nstruct RSQ\
    ...
    \ {A[i], A[i] + R[i], 1};\n KDT kdt(v);\n long long ans= 0;\n for (int i= 0; i\
    \ < N; i++)\n  if (L[i] > 0) ans+= kdt.fold_cuboid(A[i] - L[i], A[i] - 1, A[i],\
    \ 0x7fffffff);\n cout << ans << \"\\n\";\n return 0;\n}\n"
  code: "#define PROBLEM \"https://yukicoder.me/problems/no/728\"\n#include <iostream>\n\
    ...
    \ i= 0; i < N; i++) v[i]= {A[i], A[i] + R[i], 1};\n KDT kdt(v);\n long long ans=\
    \ 0;\n for (int i= 0; i < N; i++)\n  if (L[i] > 0) ans+= kdt.fold_cuboid(A[i]\
    \ - L[i], A[i] - 1, A[i], 0x7fffffff);\n cout << ans << \"\\n\";\n return 0;\n\
    }"
  dependsOn:
