...
data:
  ...
  bundledCode: "#line 1 \"test/yukicoder/215.Poly.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/215\"\
    ...
    \u7D1A\u6570 div\n * @category FFT\n */\n\n// BEGIN CUT HERE\ntemplate <class\
    \ mod_t, std::size_t _Nm= 1 << 22> std::vector<mod_t> div(const std::vector<mod_t>\
    ...
    \ static constexpr int B= 29;\n const int n= p.size(), len= pw2(n), R= len < TH2\
    ...
    \ --j;) GAp::bf[i]-= GAp::bf[i - j] * GAq::bf[j];\n  return std::vector<mod_t>(GAp::bf,\
