...
data:
  ...
  bundledCode: "#line 1 \"test/yukicoder/1303.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/1303\"\
    ...
    \ }\n};\n#line 9 \"test/yukicoder/1303.test.cpp\"\nusing namespace std;\nsigned\
    \ main() {\n cin.tie(0);\n ios::sync_with_stdio(0);\n using Mint= ModInt<998244353>;\n\
    \ int N, M;\n cin >> N >> M;\n Matrix<Mint> L(N, N);\n UnionFind uf(N);\n int\
    \ u[M], v[M];\n for (int i= 0; i < M; ++i) cin >> u[i] >> v[i], uf.unite(--u[i],\
    \ --v[i]), L[u[i]][v[i]]-= 1, L[v[i]][u[i]]-= 1, L[u[i]][u[i]]+= 1, L[v[i]][v[i]]+=\
    \ 1;\n if (uf.size(0) == N) {  // connected\n  Matrix<Mint> M1(N, N);\n  for (int\
    \ i= N; i--;)\n   for (int j= i; j--;)\n    if (L[i][j] == 0) M1[i][j]-= 1, M1[j][i]-=\
    \ 1, M1[i][i]+= 1, M1[j][j]+= 1;\n  vector<int> p(N - 1);\n  iota(p.begin(), p.end(),\
    \ 0);\n  auto poly= det_of_first_degree_poly_mat(L.submatrix(p, p), M1.submatrix(p,\
    \ p));\n  cout << 0 << '\\n';\n  cout << poly[0] + poly[1] << '\\n';\n } else\
    \ {  // unconnected\n  int fuben= 0, cnt= 0, mx= 0;\n  for (int i= N; i--;)\n\
    \   for (int j= N; j--;) fuben+= !uf.same(i, j);\n  for (int i= N; i--;)\n   for\
    \ (int j= i; j--;)\n    if (!uf.same(i, j)) {\n     if (int s= uf.size(i) * uf.size(j);\
    \ mx < s) mx= s, cnt= 1;\n     else if (mx == s) ++cnt;\n    }\n  cout << fuben\
    \ - mx * 2 << '\\n';\n  vector<int> vs[N];\n  for (int i= 0; i < N; ++i) vs[uf.root(i)].push_back(i);\n\
    \  Mint ans= cnt;\n  for (int i= 0; i < N; ++i)\n   if (int n= vs[i].size(); n\
    \ != 0) {\n    vs[i].pop_back();\n    ans*= characteristic_polynomial(L.submatrix(vs[i],\
    \ vs[i]) * -1)[0];\n   }\n  cout << ans << '\\n';\n }\n return 0;\n}\n"
  code: "#define PROBLEM \"https://yukicoder.me/problems/no/1303\"\n#include <iostream>\n\
    ...
    \ u[i] >> v[i], uf.unite(--u[i], --v[i]), L[u[i]][v[i]]-= 1, L[v[i]][u[i]]-= 1,\
    \ L[u[i]][u[i]]+= 1, L[v[i]][v[i]]+= 1;\n if (uf.size(0) == N) {  // connected\n\
    \  Matrix<Mint> M1(N, N);\n  for (int i= N; i--;)\n   for (int j= i; j--;)\n \
    \   if (L[i][j] == 0) M1[i][j]-= 1, M1[j][i]-= 1, M1[i][i]+= 1, M1[j][j]+= 1;\n\
    \  vector<int> p(N - 1);\n  iota(p.begin(), p.end(), 0);\n  auto poly= det_of_first_degree_poly_mat(L.submatrix(p,\
    \ p), M1.submatrix(p, p));\n  cout << 0 << '\\n';\n  cout << poly[0] + poly[1]\
    \ << '\\n';\n } else {  // unconnected\n  int fuben= 0, cnt= 0, mx= 0;\n  for\
    \ (int i= N; i--;)\n   for (int j= N; j--;) fuben+= !uf.same(i, j);\n  for (int\
    \ i= N; i--;)\n   for (int j= i; j--;)\n    if (!uf.same(i, j)) {\n     if (int\
    \ s= uf.size(i) * uf.size(j); mx < s) mx= s, cnt= 1;\n     else if (mx == s) ++cnt;\n\
    \    }\n  cout << fuben - mx * 2 << '\\n';\n  vector<int> vs[N];\n  for (int i=\
    \ 0; i < N; ++i) vs[uf.root(i)].push_back(i);\n  Mint ans= cnt;\n  for (int i=\
