...
data:
  ...
  bundledCode: "#line 1 \"test/yukicoder/235.HLD.test.cpp\"\n#define PROBLEM \"https://yukicoder.me/problems/no/235\"\
    ...
    \ T op(const T &vl, const T &vr) { return {vl.val + vr.val, vl.coef + vr.coef};\
    ...
    \ i= 0; i < N; i++) cin >> C[i];\n for (int i= 0; i < N - 1; i++) {\n  int A,\
    \ B;\n  cin >> A >> B;\n  A--, B--;\n  hld.add_edge(A, B);\n }\n hld.build(0);\n\
    \ SegmentTree_Beats<Mono> seg(N);\n for (int i= 0; i < N; i++) seg.unsafe_set(hld[i],\
    \ {S[i], C[i]});\n seg.rebuild();\n auto q= [&](int a, int b) { return seg.fold(a,\
    \ b); };\n int Q;\n cin >> Q;\n while (Q--) {\n  int op, X, Y;\n  cin >> op >>\
    \ X >> Y;\n  X--, Y--;\n  if (op) {\n   cout << hld.fold_path(X, Y, q, Mono::op,\
    \ Mono::ti()).val << endl;\n  } else {\n   Mint Z;\n   cin >> Z;\n   auto upd=\
    \ [&](int a, int b) { seg.apply(a, b, Z); };\n   hld.apply_path(X, Y, upd);\n\
    \  }\n }\n return 0;\n}\n"
