...
data:
  ...
  attributes:
    ...
    PROBLEM: https://onlinejudge.u-aizu.ac.jp/problems/0322
    links:
    - https://onlinejudge.u-aizu.ac.jp/problems/0322
  bundledCode: "#line 1 \"test/aoj/0322.RH.test.cpp\"\n#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/0322\"\
    ...
    \ l, int64_t r) { return l + rng() % (r - l); }\n#line 12 \"test/aoj/0322.RH.test.cpp\"\
    ...
    \ (int j= 0; j < n; ++j) dic_s[n - j].push_back(rh.sub(j).hash());\n  dic[n].push_back(rh.hash());\n\
    \ }\n for (int n= 1; n <= 200; ++n) {\n  sort(dic_p[n].begin(), dic_p[n].end());\n\
    \  sort(dic_s[n].begin(), dic_s[n].end());\n  sort(dic[n].begin(), dic[n].end());\n\
    \ }\n auto count= [](vector<K> *dc, K x) { return upper_bound(dc->begin(), dc->end(),\
    \ x) - lower_bound(dc->begin(), dc->end(), x); };\n RH chars(\"abcdefghijklmnopqrstuvwxyz\"\
    );\n for (int i= 0; i < M; ++i) {\n  string slate;\n  cin >> slate;\n  int m=\
    \ slate.length();\n  int wi= -1;\n  for (int j= 0; j < m; ++j)\n   if (slate[j]\
    \ == '?') wi= j;\n  RH rh(slate);\n  vector<K> *dc;\n  if (slate[0] == '*') {\n\
    \   rh= rh.sub(1), --m;\n   if (wi != -1) --wi;\n   dc= &dic_s[m];\n  } else if\
    \ (slate.back() == '*') {\n   rh= rh.sub(0, m - 1), --m;\n   dc= &dic_p[m];\n\
    \  } else {\n   dc= &dic[m];\n  }\n  int ans= 0;\n  if (wi == -1) ans= count(dc,\
    \ rh.hash());\n  else\n   for (int c= 26; c--;) ans+= count(dc, concat_hash(rh.sub(0,\
    \ wi), chars.sub(c, 1), rh.sub(wi + 1)));\n  cout << ans << '\\n';\n }\n return\
    \ 0;\n}\n"
  code: "#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/0322\"\n\n//\
    \ multiset \u4F7F\u3063\u305F\u3089 MLE \u306B\u306A\u3063\u305F\n\n#include <iostream>\n\
  ...
  verifiedWith: []
documentation_of: test/aoj/0322.RH.test.cpp
layout: document
redirect_from:
- /verify/test/aoj/0322.RH.test.cpp
- /verify/test/aoj/0322.RH.test.cpp.html
