...
data:
  ...
  code: "#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/2345\"\n#define\
    ...
    \ << (g.connected_component_num().back() == 1) << '\\n';\n  } else {\n    auto\
    \ e = g.edge_space_rank();\n    vector<double> h(e.size());\n    for (int s =\
    \ e.size(); s--;) h[s] = pow(100. / P, e[s]);\n    auto f = SetPowerSeries<>::log(h);\n\
