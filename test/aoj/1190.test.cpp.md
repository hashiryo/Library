...
data:
  ...
  attributes:
    ...
    ERROR: '0.000001'
    PROBLEM: https://onlinejudge.u-aizu.ac.jp/problems/1190
    links:
    - https://onlinejudge.u-aizu.ac.jp/problems/1190
  bundledCode: "#line 1 \"test/aoj/1190.test.cpp\"\n#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/1190\"\
    \n#define ERROR \"0.000001\"\n#include <iostream>\n#include <vector>\n#include\
    \ <algorithm>\n#include <cmath>\n#line 3 \"src/Geometry/Point.hpp\"\n#include\
    \ <fstream>\n#include <iomanip>\n#line 6 \"src/Geometry/Point.hpp\"\n#include\
    \ <cassert>\nnamespace geo {\nusing namespace std;\nstruct Visualizer {\n ofstream\
    ...
    \ + c.r * f, !f));\n  }\n }\n return ls;\n}\n}\n#line 8 \"test/aoj/1190.test.cpp\"\
    \nusing namespace std;\nsigned main() {\n cin.tie(0);\n ios::sync_with_stdio(false);\n\
    \ using namespace geo;\n cout << fixed << setprecision(12);\n using R= long double;\n\
    \ for (int N; cin >> N && N;) {\n  vector<Circle<R>> cs(N);\n  for (int i= 0;\
    \ i < N; i++) cin >> cs[i].o >> cs[i].r;\n  vector<Point<R>> ps;\n  for (int i=\
    \ N; i--;) {\n   ps.push_back(cs[i].o);\n   for (int j= i; j--;) {\n    auto cp=\
    \ cross_points(cs[i], cs[j]);\n    if (cp.size() <= 1) continue;\n    ps.push_back((cp[0]\
    \ + cp[1]) / 2);\n    Segment s= {cp[0], cp[1]};\n    for (int k= j; k--;) {\n\
    \     cp= cross_points(cs[i], cs[k]);\n     if (cp.size() <= 1) continue;\n  \
    \   cp= cross_points(s, Segment(cp[0], cp[1]));\n     if (!cp.size()) continue;\n\
    \     ps.push_back(cp[0]);\n    }\n   }\n  }\n  R ans= 0;\n  for (const auto &p:\
    \ ps) {\n   R a= 1e10;\n   for (const auto &c: cs) a= min(a, c.r * c.r - dist2(c.o,\
    \ p));\n   ans= max(ans, a);\n  }\n  cout << sqrt(ans) << '\\n';\n }\n return\
    \ 0;\n}\n"
  code: "#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/1190\"\n#define\
    \ ERROR \"0.000001\"\n#include <iostream>\n#include <vector>\n#include <algorithm>\n\
    ...
    \ cs[j]);\n    if (cp.size() <= 1) continue;\n    ps.push_back((cp[0] + cp[1])\
    \ / 2);\n    Segment s= {cp[0], cp[1]};\n    for (int k= j; k--;) {\n     cp=\
    \ cross_points(cs[i], cs[k]);\n     if (cp.size() <= 1) continue;\n     cp= cross_points(s,\
    \ Segment(cp[0], cp[1]));\n     if (!cp.size()) continue;\n     ps.push_back(cp[0]);\n\
    \    }\n   }\n  }\n  R ans= 0;\n  for (const auto &p: ps) {\n   R a= 1e10;\n \
