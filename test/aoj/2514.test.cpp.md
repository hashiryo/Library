...
data:
  ...
  bundledCode: "#line 1 \"test/aoj/2514.test.cpp\"\n#define PROBLEM \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=2514\"\
    ...
    \  int M= ps.size();\n  Real dis[M][M];\n  for (int i= 0; i < M; i++) fill(dis[i],\
    \ dis[i] + M, INF), dis[i][i]= 0;\n  for (int i= 0; i < M; i++)\n   for (int j=\
    \ i + 1; j < M; j++)\n    if (gs[a].contains({ps[i], ps[j]}) && h.contains({ps[i],\
    \ ps[j]})) dis[i][j]= dis[j][i]= dist(ps[i], ps[j]);\n  for (int k= 0; k < M;\
    \ k++)\n   for (int i= 0; i < M; i++)\n    for (int j= 0; j < M; j++) dis[i][j]=\
    \ min(dis[i][j], dis[i][k] + dis[k][j]);\n  if (dis[0][1] == INF) cout << \"impossible\"\
  ...
  code: "#define PROBLEM \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=2514\"\
    ...
    \ p: h)\n   if (gs[a].where(p) != OUT) ps.push_back(p);\n  int M= ps.size();\n\
    \  Real dis[M][M];\n  for (int i= 0; i < M; i++) fill(dis[i], dis[i] + M, INF),\
    \ dis[i][i]= 0;\n  for (int i= 0; i < M; i++)\n   for (int j= i + 1; j < M; j++)\n\
    \    if (gs[a].contains({ps[i], ps[j]}) && h.contains({ps[i], ps[j]})) dis[i][j]=\
    \ dis[j][i]= dist(ps[i], ps[j]);\n  for (int k= 0; k < M; k++)\n   for (int i=\
    \ 0; i < M; i++)\n    for (int j= 0; j < M; j++) dis[i][j]= min(dis[i][j], dis[i][k]\
