...
data:
  ...
  bundledCode: "#line 1 \"test/aoj/2614.RH.test.cpp\"\n#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/2614\"\
    ...
    \ + n; ++i) ret+= (char)str[i];\n  return ret;\n }\n};\n#line 9 \"test/aoj/2614.RH.test.cpp\"\
    \nusing namespace std;\nsigned main() {\n cin.tie(0);\n ios::sync_with_stdio(0);\n\
    \ using Mint= ModInt<998244353>;\n using K= CartesianProduct<Mint, Mint>;\n using\
    \ RH= RollingHash<K, char>;\n RH::init({rng(), rng()});\n string S, T;\n cin >>\
    \ S >> T;\n RH rh(S), rhT(T);\n int N= S.length(), M= T.length();\n multiset<K>\
    \ hs;\n for (int i= 0; i + M <= N; i++) hs.insert(rh.sub(i, M).hash());\n vector<char>\
    \ chars(52);\n for (int i= 0; i < 26; i++) chars[i]= 'a' + i;\n for (int i= 0;\
    \ i < 26; i++) chars[i + 26]= 'A' + i;\n RH rhC(chars);\n int ans= 0;\n for (int\
    \ i= 0; i < M; ++i)\n  for (int j= 0; j < 52; ++j)\n   if (T[i] != chars[j]) ans+=\
    \ hs.count(concat_hash(rhT.sub(0, i), rhC.sub(j, 1), rhT.sub(i + 1)));\n cout\
    \ << ans << '\\n';\n return 0;\n}\n"
  code: "#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/2614\"\n#include\
    ...
    \ rng()});\n string S, T;\n cin >> S >> T;\n RH rh(S), rhT(T);\n int N= S.length(),\
    \ M= T.length();\n multiset<K> hs;\n for (int i= 0; i + M <= N; i++) hs.insert(rh.sub(i,\
    \ M).hash());\n vector<char> chars(52);\n for (int i= 0; i < 26; i++) chars[i]=\
    \ 'a' + i;\n for (int i= 0; i < 26; i++) chars[i + 26]= 'A' + i;\n RH rhC(chars);\n\
    \ int ans= 0;\n for (int i= 0; i < M; ++i)\n  for (int j= 0; j < 52; ++j)\n  \
    \ if (T[i] != chars[j]) ans+= hs.count(concat_hash(rhT.sub(0, i), rhC.sub(j, 1),\
    \ rhT.sub(i + 1)));\n cout << ans << '\\n';\n return 0;\n}"
  dependsOn:
  - src/Math/ModInt.hpp
  ...
  verifiedWith: []
documentation_of: test/aoj/2614.RH.test.cpp
layout: document
redirect_from:
- /verify/test/aoj/2614.RH.test.cpp
- /verify/test/aoj/2614.RH.test.cpp.html
