...
data:
  ...
  attributes:
    ...
    ERROR: '0.000000001'
    PROBLEM: https://onlinejudge.u-aizu.ac.jp/problems/2256
    links:
    - https://onlinejudge.u-aizu.ac.jp/problems/2256
  bundledCode: "#line 1 \"test/aoj/2256.test.cpp\"\n#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/2256\"\
    ...
    \ l= line_through(p0, p);\n    if (!check(l)) continue;\n    auto cp= cross_points(right,\
    \ l);\n    if (!cp.size()) continue;\n    mn= min(mn, cp[0].y), mx= max(mx, cp[0].y);\n\
    \   }\n   if (mx < 0) return R(0);\n   return mx - mn;\n  };\n  R ans= 0;\n  for\
    \ (int i= xs.size(); --i;) ans+= (xs[i] - xs[i - 1]) * f((xs[i] + xs[i - 1]) /\
    \ 2);\n  ans/= H * H;\n  cout << ans << '\\n';\n }\n return 0;\n}\n"
  code: "#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/2256\"\n// Rational\
    \ \u3060\u3068 WA \u304B TLE\n#define ERROR \"0.000000001\"\n#include <iostream>\n\
    #include <iomanip>\n#include <vector>\n#include <algorithm>\n#include \"src/Misc/compress.hpp\"\
    \n#include \"src/Geometry/Segment.hpp\"\nusing namespace std;\nsigned main() {\n\
    \ cin.tie(0);\n ios::sync_with_stdio(false);\n using namespace geo;\n using R=\
    \ long double;\n cout << fixed << setprecision(12);\n for (int W, H, N; cin >>\
    \ W >> H >> N && N;) {\n  Segment<R> left({0, 0}, {0, R(H)}), right({R(W), 0},\
    \ {R(W), R(H)});\n  vector<Point<R>> ps(2 * N);\n  for (int i= 0; i < 2 * N; ++i)\
    \ cin >> ps[i];\n  auto check= [&](const Line<R> &l) {\n   int on= 0, dif= 0;\n\
    \   for (auto &p: ps) {\n    int w= l.where(p);\n    on+= (w == 0), dif+= w;\n\
    \   }\n   return abs(dif) <= on;\n  };\n  vector<R> xs;\n  auto qs= ps;\n  qs.emplace_back(right.p),\
    \ qs.emplace_back(right.q);\n  compress(qs);\n  for (int i= qs.size(); i--;)\n\
    ...
    \ min(mn, cp[0].y), mx= max(mx, cp[0].y);\n   }\n   if (mx < 0) return R(0);\n\
    \   return mx - mn;\n  };\n  R ans= 0;\n  for (int i= xs.size(); --i;) ans+= (xs[i]\
    \ - xs[i - 1]) * f((xs[i] + xs[i - 1]) / 2);\n  ans/= H * H;\n  cout << ans <<\
    \ '\\n';\n }\n return 0;\n}"
  dependsOn:
