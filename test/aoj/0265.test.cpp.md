...
data:
  ...
  bundledCode: "#line 1 \"test/aoj/0265.test.cpp\"\n#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/challenges/sources/PCK/Prelim/0265\"\
    ...
    \   }\n   compress(ps);\n   for (int i= ps.size(); --i;) ss.emplace_back(ps[i\
    \ - 1], ps[i]);\n  }\n  int m= ss.size();\n  u128 flg[n], all= (u128(1) << m)\
    \ - 1;\n  fill_n(flg, n, 0);\n  for (int i= n; i--;)\n   for (int j= m; j--;)\
    \ flg[i]|= u128(g.in({ss[j].p, g[i]}) & g.in({ss[j].q, g[i]})) << j;\n  int ans=\
  ...
  code: "#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/challenges/sources/PCK/Prelim/0265\"\
    ...
    \ n; i--;)\n   for (int j= m; j--;) flg[i]|= u128(g.in({ss[j].p, g[i]}) & g.in({ss[j].q,\
    \ g[i]})) << j;\n  int ans= n;\n  for (int s= 1 << n; --s;) {\n   int c= __builtin_popcount(s);\n\
    \   if (ans <= c) continue;\n   u128 f= 0;\n   for (int i= n; i--;)\n    if ((s\
    \ >> i) & 1) f|= flg[i];\n   if (f == all) ans= c;\n  }\n  cout << ans << '\\\
    n';\n }\n return 0;\n}"
