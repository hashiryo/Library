...
data:
  ...
  attributes:
    ...
    ERROR: '0.00001'
    PROBLEM: http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=1171
    links:
    - http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=1171
  bundledCode: "#line 1 \"test/aoj/1171.test.cpp\"\n#define PROBLEM \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=1171\"\
    ...
    \ c = ccw(ps[ps.size() - 2], ps.back(), cp[0]);\n      if (c == ONLINE_BACK ||\
    \ c == ON_SEGMENT) return INF;\n    }\n    ps.push_back(cp[0]);\n    Line l =\
    \ {ts[i].p1, ts[i].p2};\n    for (Segment &t : ts) t = l.reflect(t);\n  }\n  ps.push_back(target);\n\
    \  ts = ss;\n  for (int k = 0; k + 1 < ps.size(); k++) {\n    Segment s = {ps[k],\
    \ ps[k + 1]};\n    for (Segment t : ts)\n      if (intersect(s, t) == CROSSING)\
  ...
  code: "#define PROBLEM \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=1171\"\
    ...
    \ return INF;\n    if (ps.size() >= 2) {\n      auto c = ccw(ps[ps.size() - 2],\
    \ ps.back(), cp[0]);\n      if (c == ONLINE_BACK || c == ON_SEGMENT) return INF;\n\
    \    }\n    ps.push_back(cp[0]);\n    Line l = {ts[i].p1, ts[i].p2};\n    for\
    \ (Segment &t : ts) t = l.reflect(t);\n  }\n  ps.push_back(target);\n  ts = ss;\n\
