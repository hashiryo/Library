...
data:
  ...
  bundledCode: "#line 1 \"test/aoj/0355.test.cpp\"\n#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/0355\"\
    ...
    \ n) {\n bs= {rng(), rng()};\n iv= K(1) / (bs - 1);\n pw[0]= 1;\n for (int i=\
    \ 1; i <= n; ++i) pw[i]= pw[i - 1] * bs;\n}\nstruct M {\n struct T {\n  K val;\n\
    \  int sz;\n };\n using E= char;\n static T ti() { return {0, 0}; }\n static T\
    \ op(const T& l, const T& r) { return {l.val * pw[r.sz] + r.val, l.sz + r.sz};\
    ...
    \ m + 1;\n   while (ng - ok > 1) {\n    int x= (ok + ng) / 2;\n    if (seg.fold(a,\
    \ a + x).val == seg.fold(c, c + x).val) ok= x;\n    else ng= x;\n   }\n   if (ok\
    \ == m) {\n    cout << (s < t ? \"s\" : s > t ? \"t\" : \"e\") << '\\n';\n   }\
  ...
  code: "#define PROBLEM \"https://onlinejudge.u-aizu.ac.jp/problems/0355\"\n\n//\
    ...
    \ a, b, c, d;\n   cin >> a >> b >> c >> d, --a, --c;\n   int s= b - a, t= d -\
    \ c, m= min(s, t);\n   int ok= 0, ng= m + 1;\n   while (ng - ok > 1) {\n    int\
    \ x= (ok + ng) / 2;\n    if (seg.fold(a, a + x).val == seg.fold(c, c + x).val)\
    \ ok= x;\n    else ng= x;\n   }\n   if (ok == m) {\n    cout << (s < t ? \"s\"\
    \ : s > t ? \"t\" : \"e\") << '\\n';\n   } else {\n    K sck= seg[a + ok].val,\
