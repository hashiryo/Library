...
data:
  ...
  bundledCode: "#line 1 \"test/aoj/2003.test.cpp\"\n#define PROBLEM \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=2003\"\
    ...
    \ get<1>(cps[0]) ? get<2>(cps[0]) : !get<2>(cps[0]);\n   int ans= 0;\n   for (int\
    \ i= 1; i < cps.size(); i++)\n    if (get<1>(cps[i]) ^ (cur == get<2>(cps[i])))\
    \ ans++, cur= !cur;\n   cout << ans << endl;\n  }\n }\n return 0;\n}\n"
  code: "#define PROBLEM \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=2003\"\
    ...
    \ cps.emplace_back(ps[0], o, l);\n  }\n  if (!cps.size()) cout << 0 << endl;\n\
    \  else {\n   sort(cps.begin(), cps.end());\n   bool cur= get<1>(cps[0]) ? get<2>(cps[0])\
