...
data:
  ...
  bundledCode: "#line 1 \"test/aoj/1242.test.cpp\"\n#define PROBLEM \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=1242\"\
    ...
    \ if (x < r) {\n     ans+= r - x;\n     x= r;\n    }\n   }\n  }\n  cout << ans\
    \ << endl;\n }\n return 0;\n}\n"
  code: "#define PROBLEM \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=1242\"\
    ...
    \ int>> v;\n   Line l= {{0, Real(y)}, {1, Real(y)}}, m= {{0, Real(y) + 1}, {1,\
    \ Real(y) + 1}};\n   for (int i= 0; i < N; i++) {\n    if ((g[i].y <= y && g[g.next(i)].y\
    \ <= y) || (g[i].y >= y + 1 && g[g.next(i)].y >= y + 1)) continue;\n    Point\
    \ p= cross_points(Segment({g[i], g[g.next(i)]}), l)[0];\n    Point q= cross_points(Segment({g[i],\
    \ g[g.next(i)]}), m)[0];\n    Real left= min(p.x, q.x), right= max(p.x, q.x);\n\
    \    int li= left, ri= right;\n    for (int i= -1; i <= 1; i++)\n     if (0 <=\
    \ sgn(left - li - i) && sgn(left - li - i - 1) < 0) {\n      li= li + i;\n   \
    \   break;\n     }\n    for (int i= -1; i <= 1; i++)\n     if (0 < sgn(right -\
    \ ri - i + 1) && sgn(right - ri - i) <= 0) {\n      ri= ri + i;\n      break;\n\
  ...
  verifiedWith: []
documentation_of: test/aoj/1242.test.cpp
layout: document
redirect_from:
- /verify/test/aoj/1242.test.cpp
- /verify/test/aoj/1242.test.cpp.html
