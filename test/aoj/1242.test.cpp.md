...
data:
  ...
  bundledCode: "#line 1 \"test/aoj/1242.test.cpp\"\n#define PROBLEM \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=1242\"\
    ...
    \ &operator<<(Polygon g) {\n    ofs << \"Polygon\" << endl;\n    for (Point p\
    \ : g) ofs << p << endl;\n    ofs << \"...\" << endl;\n    return *this;\n  }\n\
    };\n\n}  // namespace geometry\n#line 8 \"test/aoj/1242.test.cpp\"\n#undef call_from_test\n\
    \nsigned main() {\n  cin.tie(0);\n  ios::sync_with_stdio(false);\n  using namespace\
    \ geometry;\n  for (int N; cin >> N && N;) {\n    Polygon g(N);\n    int ymin\
    \ = 2001, ymax = -2001;\n    for (int i = 0; i < N; i++) {\n      cin >> g[i];\n\
    \      ymin = min(ymin, int(g[i].y) - 1);\n      ymax = max(ymax, int(g[i].y)\
    ...
    \ q = cross_points(Segment({g[i], g[g.next(i)]}), m)[0];\n        Real left =\
    \ min(p.x, q.x), right = max(p.x, q.x);\n        int li = left, ri = right;\n\
    \        for (int i = -1; i <= 1; i++)\n          if (0 <= sgn(left - li - i)\
    \ && sgn(left - li - i - 1) < 0) {\n            li = li + i;\n            break;\n\
    \          }\n        for (int i = -1; i <= 1; i++)\n          if (0 < sgn(right\
  ...
  code: "#define PROBLEM \"http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=1242\"\
    ...
    \  for (int N; cin >> N && N;) {\n    Polygon g(N);\n    int ymin = 2001, ymax\
    \ = -2001;\n    for (int i = 0; i < N; i++) {\n      cin >> g[i];\n      ymin\
    \ = min(ymin, int(g[i].y) - 1);\n      ymax = max(ymax, int(g[i].y) + 1);\n  \
    \  }\n    int ans = 0;\n    for (int y = ymin; y <= ymax; y++) {\n      vector<pair<int,\
    \ int>> v;\n      Line l = {{0, Real(y)}, {1, Real(y)}},\n           m = {{0,\
    \ Real(y) + 1}, {1, Real(y) + 1}};\n      for (int i = 0; i < N; i++) {\n    \
    \    if ((g[i].y <= y && g[g.next(i)].y <= y)\n            || (g[i].y >= y + 1\
    \ && g[g.next(i)].y >= y + 1))\n          continue;\n        Point p = cross_points(Segment({g[i],\
    \ g[g.next(i)]}), l)[0];\n        Point q = cross_points(Segment({g[i], g[g.next(i)]}),\
    \ m)[0];\n        Real left = min(p.x, q.x), right = max(p.x, q.x);\n        int\
    \ li = left, ri = right;\n        for (int i = -1; i <= 1; i++)\n          if\
    \ (0 <= sgn(left - li - i) && sgn(left - li - i - 1) < 0) {\n            li =\
    \ li + i;\n            break;\n          }\n        for (int i = -1; i <= 1; i++)\n\
