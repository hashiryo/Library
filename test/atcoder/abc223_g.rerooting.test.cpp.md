...
data:
  ...
  code: "#define PROBLEM \"https://atcoder.jp/contests/abc223/tasks/abc223_g\"\n#include\
    ...
    \ put_vertex= [&](int, const Data& d) { return Data{d[0], d[0] + d[1]}; };\n Rerooting<Data>\
    \ dp(g, put_edge, op, Data{0, 0}, put_vertex);\n int mx= 0, ans= 0;\n for (auto\
    \ [x, y]: dp) mx= max({mx, x, y});\n for (auto [x, y]: dp) ans+= (mx == x);\n\
    \ cout << ans << '\\n';\n return 0;\n}"
  dependsOn:
  - src/Graph/Graph.hpp
