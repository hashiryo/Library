...
data:
  ...
  code: "#define PROBLEM \"https://atcoder.jp/contests/abc272/tasks/abc272_f\"\n#include\
    ...
    \ RH::init({rng(), rng()});\n int N;\n cin >> N;\n string S, T;\n cin >> S >>\
    \ T;\n S+= S, T+= T;\n RH rhS(S), rhT(T);\n vector<RH> rh(N);\n for (int i= 0;\
    \ i < N; ++i) rh[i]= rhS.sub(i, N);\n stable_sort(rh.begin(), rh.end());\n long\
    \ long ans= 0;\n for (int i= 0; i < N; ++i) ans+= upper_bound(rh.begin(), rh.end(),\
    \ rhT.sub(i, N)) - rh.begin();\n cout << ans << '\\n';\n return 0;\n}"
  dependsOn:
  - src/String/RollingHash.hpp
...
layout: document
redirect_from:
- /verify/test/atcoder/abc272_f.RH.test.cpp
- /verify/test/atcoder/abc272_f.RH.test.cpp.html
