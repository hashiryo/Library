...
data:
  ...
  bundledCode: "#line 1 \"test/atcoder/arc129_e.Dinic.test.cpp\"\n#define PROBLEM\
    ...
    \ sol);\n}\n#line 6 \"test/atcoder/arc129_e.Dinic.test.cpp\"\nusing namespace\
  ...
  code: "#define PROBLEM \"https://atcoder.jp/contests/arc129/tasks/arc129_e\"\n#include\
    ...
    \ < M; k++) cin >> A[i][k] >> C[i][k];\n vector<vector<long long>> W(N, vector<long\
    \ long>(N));\n for (int i= 0; i < N; i++)\n  for (int j= i + 1; j < N; j++) cin\
    \ >> W[i][j];\n auto theta= [&](int i, int xi) { return C[i][xi]; };\n auto phi=\
    \ [&](int i, int j, int xi, int xj) { return abs(A[i][xi] - A[j][xj]) * W[i][j];\
    \ };\n auto [ans, x]= monge_mincut<MF>(N, M, theta, phi);\n cout << ans << '\\\
    n';\n return 0;\n}"
  dependsOn:
