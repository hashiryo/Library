...
data:
  ...
  bundledCode: "#line 1 \"test/atcoder/abc133_f.DynSeg.test.cpp\"\n#define PROBLEM\
    ...
    \ down.rbegin(), down.rend()), up;\n }\n};\n#line 10 \"test/atcoder/abc133_f.DynSeg.test.cpp\"\
  ...
  code: "#define PROBLEM \"https://atcoder.jp/contests/abc133/tasks/abc133_f\"\n\n\
    ...
    \ dep[u]= dep[v] + d[e];\n  }\n }\n for (int i= 0; i < Q; i++) {\n  int x, y,\
    \ u, v;\n  cin >> x >> y >> u >> v;\n  int lca= hld.lca(--u, --v);\n  cout <<\
    \ dep[u] + dep[v] - 2 * dep[lca] + y * (Arr1[u].get(x) + Arr1[v].get(x) - 2 *\
    \ Arr1[lca].get(x)) - (Arr2[u].get(x) + Arr2[v].get(x) - 2 * Arr2[lca].get(x))\
    \ << '\\n';\n }\n return 0;\n}"
  dependsOn:
  - src/DataStructure/SegmentTree_Dynamic.hpp
